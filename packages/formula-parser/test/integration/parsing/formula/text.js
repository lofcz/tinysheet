...
describe(".parse() text formulas", () => {
  ...
  it("CODE", () => {
    ...
    expect(
      parser.parse('CODE("a")')
    ).toMatchObject({ error: null, result: 97 });
  });

  it("CONCATENATE", () => {
    expect(
      parser.parse("CONCATENATE()")
    ).toMatchObject({ error: null, result: "" });
    expect(
      parser.parse('CONCATENATE("a")')
    ).toMatchObject({ error: null, result: "a" });
    expect(
      parser.parse('CONCATENATE("a", 1)')
    ).toMatchObject({ error: null, result: "a1" });
    expect(
      parser.parse('CONCATENATE("a", 1, TRUE)')
    ).toMatchObject({ error: null, result: "a1TRUE" });
  });
  ...
  xit("DOLLAR", () => {
    ...
    expect(
      parser.parse("DOLLAR(1100, -2)")
    ).toMatchObject({ error: null, result: "$1,100" });
  });
  ...
  it("EXACT", () => {
    ...
    expect(
      parser.parse("EXACT(1100)")
    ).toMatchObject({ error: "#N/A", result: null });
    expect(
      parser.parse("EXACT(1100, -2)")
    ).toMatchObject({ error: null, result: false });
    expect(
      parser.parse("EXACT(1100, 1100)")
  ...
  xit("FIXED", () => {
    ...
    expect(
      parser.parse("FIXED(12345.11, -1)")
    ).toMatchObject({ error: null, result: "12,350" });
    expect(
      parser.parse("FIXED(12345.11, 0)")
    ).toMatchObject({ error: null, result: "12,345" });
    expect(
      parser.parse("FIXED(12345.11, 0, TRUE)")
    ).toMatchObject({ error: null, result: "12345" });
    expect(
      parser.parse("FIXED(12345.11, 4, TRUE)")
    ).toMatchObject({ error: null, result: "12345.1100" });
  });

  it("HTML2TEXT", () => {
    expect(
      parser.parse("HTML2TEXT()")
    ).toMatchObject({ error: "#NAME?", result: null });
    expect(
      parser.parse('HTML2TEXT("Click <a>Link</a>")')
    ).toMatchObject({ error: "#NAME?", result: null });
  });
  ...
  it("LEFT", () => {
    ...
    expect(
      parser.parse('LEFT("Foo Bar")')
    ).toMatchObject({ error: null, result: "F" });
    expect(
      parser.parse('LEFT("Foo Bar", 3)')
  ...
  it("LEN", () => {
    ...
    expect(
      parser.parse("LEN(TRUE)")
    ).toMatchObject({ error: null, result: 4 });
    expect(
      parser.parse("LEN(1023)")
    ).toMatchObject({ error: null, result: 4 });
    expect(
      parser.parse('LEN("Foo Bar")')
  ...
  it("LOWER", () => {
    ...
    expect(
      parser.parse('LOWER("Foo Bar")')
    ).toMatchObject({ error: null, result: "foo bar" });
  });
  ...
  it("MID", () => {
    ...
    expect(
      parser.parse('MID("Foo Bar", 2)')
    ).toMatchObject({ error: "#VALUE!", result: null });
    expect(
      parser.parse('MID("Foo Bar", 2, 5)')
    ).toMatchObject({ error: null, result: "oo Ba" });
  });
  ...
  it("PROPER", () => {
    ...
    expect(
      parser.parse("PROPER(TRUE)")
    ).toMatchObject({ error: null, result: "True" });
    expect(
      parser.parse("PROPER(1234)")
    ).toMatchObject({ error: null, result: "1234" });
  ...
  it("REPT", () => {
    ...
    expect(parser.parse('REPT("foo ")')).toMatchObject({ error: null, result: "" });
  ...
  xit("SPLIT", () => {
    ...
    expect(parser.parse('SPLIT("foo bar baz")')).toMatchObject({ error: "#NAME?", result: null });
    expect(parser.parse('SPLIT("foo bar baz", " ")')).toMatchObject({ error: "#NAME?", result: null });
  });

  it("SUBSTITUTE", () => {
    expect(
      parser.parse("SUBSTITUTE()")
    ).toMatchObject({ error: "#N/A", result: null });
  ...
  it("UNICHAR", () => {
    expect(
      parser.parse("UNICHAR()")
    ).toMatchObject({ error: "#VALUE!", result: null });
    expect(
      parser.parse("UNICHAR(33)")
    ).toMatchObject({ error: null, result: "!" });
  });
  ...
  it("UNICODE", () => {
    ...
    expect(
      parser.parse('UNICODE("!")')
    ).toMatchObject({ error: null, result: 33 });
  });
  ...
  xit("VALUE", () => {
    ...
    expect(
      parser.parse('VALUE("$1,000")')
    ).toMatchObject({ error: null, result: 1000 });
    expect(
      parser.parse('VALUE("01:00:00")')
    ).toMatchObject({ error: "#VALUE!", result: null });
