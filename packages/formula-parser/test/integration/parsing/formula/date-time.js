...
describe(".parse() date & time formulas", () => {
  ...
  it("DAY", () => {
    ...
    expect(
      parser.parse("DAY(2958465)")
    ).toMatchObject({ error: null, result: 31 });
    expect(
      parser.parse('DAY("2958465")')
    ).toMatchObject({ error: null, result: 31 });
  });
  ...
  it("DAYS", () => {
    ...
    expect(
      parser.parse("DAYS(1, 6)")
    ).toMatchObject({ error: null, result: -5 });
    expect(
      parser.parse('DAYS("1/2/2000", "1/10/2001")')
    ).toMatchObject({ error: null, result: -374 });
  });

  it("DAYS360", () => {
    expect(
      parser.parse("DAYS360()")
    ).toMatchObject({ error: "#VALUE!", result: null });
    expect(
      parser.parse("DAYS360(1)")
    ).toMatchObject({ error: "#VALUE!", result: null });
    expect(
      parser.parse("DAYS360(1, 6)")
    ).toMatchObject({ error: null, result: 5 });
    expect(
      parser.parse('DAYS360("1/1/1901", "2/1/1901", TRUE)')
    ).toMatchObject({ error: null, result: 30 });
    expect(
  ...
  it("HOUR", () => {
    ...
    expect(
      parser.parse('HOUR("1/1/1900 16:33")')
  ...
  it("INTERVAL", () => {
    ...
    expect(
      parser.parse("INTERVAL(0)")
    ).toMatchObject({ error: "#NAME?", result: null });
    expect(
      parser.parse("INTERVAL(1)")
    ).toMatchObject({ error: "#NAME?", result: null });
    expect(
      parser.parse("INTERVAL(60)")
    ).toMatchObject({ error: "#NAME?", result: null });
    expect(
      parser.parse("INTERVAL(10000000)")
    ).toMatchObject({ error: "#NAME?", result: null });
  });

  it("ISOWEEKNUM", () => {
    expect(
      parser.parse("ISOWEEKNUM()")
    ).toMatchObject({ error: "#VALUE!", result: null });
    expect(
      parser.parse('ISOWEEKNUM("1/8/1901")')
    ).toMatchObject({ error: null, result: 2 });
    expect(
      parser.parse('ISOWEEKNUM("6/6/1902")')
    ).toMatchObject({ error: null, result: 23 });
  });
  ...
  it("MINUTE", () => {
    ...
    expect(
      parser.parse('MINUTE("1/1/1901 1:01")')
    ).toMatchObject({ error: null, result: 1 });
    expect(
      parser.parse('MINUTE("1/1/1901 15:36")')
    ).toMatchObject({ error: null, result: 36 });
  });
  ...
  it("MONTH", () => {
    ...
    expect(
      parser.parse('MONTH("2/1/1901")')
    ).toMatchObject({ error: null, result: 2 });
    expect(
      parser.parse('MONTH("10/1/1901")')
    ).toMatchObject({ error: null, result: 10 });
  });

  it("NETWORKDAYS", () => {
    expect(
      parser.parse("NETWORKDAYS()")
    ).toMatchObject({ error: "#VALUE!", result: null });
    expect(
      parser.parse('NETWORKDAYS("2/1/1901")')
    ).toMatchObject({ error: "#VALUE!", result: null });
    expect(
  ...
  it("SECOND", () => {
    ...
    expect(
      parser.parse('SECOND("2/1/1901 13:33:12")')
  ...
  it("TIMEVALUE", () => {
    expect(
      parser.parse("TIMEVALUE()")
    ).toMatchObject({ error: "#VALUE!", result: null });
    expect(
      parser.parse('TIMEVALUE("1/1/1900 00:00:00")')
    ).toMatchObject({ error: null, result: 0 });
    expect(
      parser.parse('TIMEVALUE("1/1/1900 23:00:00")')
    ).toMatchObject({ error: null, result: 0.9583333333333334 });
  });
  ...
  it("WEEKDAY", () => {
    expect(
      parser.parse("WEEKDAY()")
    ).toMatchObject({ error: "#VALUE!", result: null });
    expect(
      parser.parse('WEEKDAY("1/1/1901")')
    ).toMatchObject({ error: null, result: 3 });
    expect(
      parser.parse('WEEKDAY("1/1/1901", 2)')
    ).toMatchObject({ error: null, result: 2 });
  });

  it("WEEKNUM", () => {
    expect(
      parser.parse("WEEKNUM()")
    ).toMatchObject({ error: "#VALUE!", result: null });
    expect(
      parser.parse('WEEKNUM("2/1/1900")')
    ).toMatchObject({ error: null, result: 5 });
    expect(
      parser.parse('WEEKNUM("2/1/1909", 2)')
  ...
  it("YEAR", () => {
    ...
    expect(
      parser.parse('YEAR("1/1/1904")')
    ).toMatchObject({ error: null, result: 1904 });
    expect(
      parser.parse('YEAR("12/12/2001")')
    ).toMatchObject({ error: null, result: 2001 });
  });

  it("YEARFRAC", () => {
    expect(
      parser.parse("YEARFRAC()")
    ).toMatchObject({ error: "#VALUE!", result: null });
    expect(
      parser.parse('YEARFRAC("1/1/1904")')
    ).toMatchObject({ error: "#VALUE!", result: null });
    expect(
      parser.parse('YEARFRAC("1/1/1900", "1/2/1900")')
    ).toMatchObject({ error: null, result: 0.002777777777777778 });
  });
});
