...
describe(".parse() logical formulas", () => {
  ...
  it("AND", () => {
    ...
    expect(
      parser.parse("AND(TRUE, TRUE, FALSE)")
    ).toMatchObject({ error: null, result: false });
    expect(
      parser.parse("AND(TRUE, TRUE, TRUE)")
  ...
  it("IF", () => {
    ...
    expect(
      parser.parse("IF(TRUE, 1, 2)")
    ).toMatchObject({ error: null, result: 1 });
    expect(
      parser.parse("IF(FALSE, 1, 2)")
  ...
  it("OR", () => {
    ...
    expect(
      parser.parse("OR(TRUE, TRUE, TRUE)")
    ).toMatchObject({ error: null, result: true });
    expect(
      parser.parse("OR(TRUE, FALSE, FALSE)")
    ).toMatchObject({ error: null, result: true });
    expect(
      parser.parse("OR(FALSE, FALSE, FALSE)")
    ).toMatchObject({ error: null, result: false });
  });
  ...
  it("XOR", () => {
    ...
    expect(
      parser.parse("XOR(TRUE, TRUE)")
    ).toMatchObject({ error: null, result: false });
    expect(
      parser.parse("XOR(TRUE, FALSE)")
    ).toMatchObject({ error: null, result: true });
    expect(
      parser.parse("XOR(FALSE, TRUE)")
    ).toMatchObject({ error: null, result: true });
    expect(
      parser.parse("XOR(FALSE, FALSE)")
    ).toMatchObject({ error: null, result: false });
  });
