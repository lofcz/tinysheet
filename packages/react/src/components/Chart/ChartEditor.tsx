...
const ChartEditor: React.FC = () => {
  ...
  // the pane follows the selected element (Format <Element>, Excel)
  const tt = chartToolsLocale(context);
  const selectable = chartSelectableElements(chart);
  const element = selectable.includes(context.chartElement ?? "")
    ? (context.chartElement as string)
    : "chartArea";
  const seriesMatch = /^series:(\d+)$/.exec(element);
  const elementName = chartElementLabel(context, chart, element, tt);
  let paneTitle = tt.names.format.replace("{element}", elementName);
  if (seriesMatch) paneTitle = tt.names.formatDataSeries;
  else if (element === "categoryAxis" || element === "valueAxis")
    paneTitle = tt.names.formatAxis;
  else if (element.endsWith("AxisTitle")) paneTitle = tt.names.formatAxisTitle;
  else if (element === "majorGridlines") paneTitle = tt.names.formatGridlines;

  const setFormat = (
    el: string,
    patch: Parameters<typeof setElementFormat>[2]
  ) => update((_ctx, c) => setElementFormat(c, el, patch));
  const formatOf = (el: string) =>
    (chart.formats as Record<string, Record<string, unknown>> | undefined)?.[
      el
    ] ?? {};

  /** "<Element> Options ▾": the element list at the top (Excel). */
  const elementSelect = () => (
    <div className="fortune-chart-editor-element">
      <Select
        aria-label={tt.format.chartElements}
        value={element}
        size="sm"
        options={selectable.map((id) => ({
          value: id,
          label: chartElementLabel(context, chart, id, tt),
        }))}
        onChange={(id) =>
          setContext(
            (ctx) => {
  ...
  /** Fill / border (and text colour) of an element. */
  const formatSection = (el: string) => {
    const caps = elementCaps(el);
    const f = formatOf(el);
    const series = /^series:(\d+)$/.exec(el);
    const s = series ? chart.series[Number(series[1])] : undefined;
    const fillValue = series
      ? (s?.color ?? chartColor(chart, Number(series[1])))
      : (f.fill as string | null | undefined);
    const lineValue = series
      ? s?.outline
      : (f.line as string | null | undefined);
    return (
      <section
        className="fortune-chart-editor-section"
        data-testid="chart-format-fill"
      >
        <h3>
          {tt.names.fill} &amp; {tt.names.line}
        </h3>
        {caps.fill && (
          <ColorField
            label={tt.names.fill}
            ...
            none={fillValue === null}
            noneLabel={tt.format.noFill}
            autoLabel={tt.format.automatic}
            disabled={readonly}
            onChange={(v) => setFormat(el, { fill: v })}
          />
        )}
        {caps.line && (
          <ColorField
            label={el === "majorGridlines" ? tt.names.line : tt.names.border}
            value={lineValue ?? undefined}
            none={lineValue === null}
            noneLabel={tt.format.noOutline}
            autoLabel={tt.format.automatic}
            disabled={readonly}
            onChange={(v) => setFormat(el, { line: v })}
          />
        )}
        {caps.text && (
          <ColorField
            label={tt.names.textColor}
            value={
              (formatOf(el === "chartArea" ? "title" : el).text as
                | string
                | undefined) ?? undefined
            }
            autoLabel={tt.format.automatic}
            disabled={readonly}
            onChange={(v) => setFormat(el, { text: v ?? undefined })}
          />
        )}
      </section>
    );
  ...
  /** The options of the selected element (not the chart area). */
  const elementPane = () => {
    if (seriesMatch) {
      const i = Number(seriesMatch[1]);
      ...
      return (
        <>
          ...
          <section className="fortune-chart-editor-section">
            <h3>
              {tt.names.options.replace(
                "{element}",
                tt.names.series.replace(' "{name}"', "")
              )}
            </h3>
            {s && renderSeries(s, i)}
          </section>
    ...
    return (
      <>
        {element === "title" && (
          <section className="fortune-chart-editor-section">
            <h3>{tt.names.options.replace("{element}", elementName)}</h3>
            <CommitField
              label={tt.names.titleText}
              value={chart.title ?? ""}
              onCommit={(text) => {
                update((_ctx, c) => {
                  c.title = text;
            ...
            />
            <CheckField
              label={tt.elements.centeredOverlay}
              checked={!!chart.titleOverlay}
              disabled={readonly}
              onChange={(on) =>
                update((_ctx, c) => {
                  if (on) c.titleOverlay = true;
                  else delete c.titleOverlay;
                })
              }
            />
          </section>
        ...
        {element === "legend" && (
          <section className="fortune-chart-editor-section">
            <h3>{tt.names.options.replace("{element}", elementName)}</h3>
            <div role="radiogroup" aria-label={tt.names.position}>
              {(["top", "bottom", "left", "right"] as const).map((p) => (
                <label
                  key={p}
                  className="fortune-chart-editor-check"
                  htmlFor={`${idBase}-legend-${p}`}
                >
                  <input
                    id={`${idBase}-legend-${p}`}
                    type="radio"
                    name={`${idBase}-legend`}
                    checked={(chart.legend ?? "right") === p}
                    disabled={readonly}
                    onChange={() =>
                      update((_ctx, c) => {
                        c.legend = p;
        ...
        {axisKey && (
          <section className="fortune-chart-editor-section">
            <h3>{tt.names.options.replace("{element}", elementName)}</h3>
            <CheckField
              label={tt.names.showAxis}
              checked={
                chart.axes?.[axisKey === "valueAxis" ? "value" : "category"] !==
                false
              }
              disabled={readonly}
              onChange={(on) =>
                update((_ctx, c) => {
                  const key = axisKey === "valueAxis" ? "value" : "category";
                  const next = { ...c.axes };
                  if (on) delete next[key];
                  else next[key] = false;
                  if (Object.keys(next).length) c.axes = next;
                  else delete c.axes;
                })
              }
            />
            {axisKey === "valueAxis" && (
              <div className="fortune-chart-editor-sub">
                ...
                <div className="fortune-chart-editor-row">
                  <NumberField
                    label={tt.names.minimum}
                    value={chart.valueAxis?.min}
                    disabled={readonly}
                    onCommit={(min) =>
                      update((_ctx, c) => {
                        c.valueAxis = { ...c.valueAxis, min };
                      })
                    }
                  />
                  <NumberField
                    label={tt.names.maximum}
                    value={chart.valueAxis?.max}
                    disabled={readonly}
                    onCommit={(max) =>
                      update((_ctx, c) => {
                        c.valueAxis = { ...c.valueAxis, max };
                      })
                    }
                  />
                </div>
                <NumberField
                  label={tt.names.majorUnit}
                  value={chart.valueAxis?.majorUnit}
                  min={0}
                  disabled={readonly}
                  onCommit={(majorUnit) =>
                    update((_ctx, c) => {
                      c.valueAxis = { ...c.valueAxis, majorUnit };
                    })
                  }
                />
              </div>
        ...
        {element.endsWith("AxisTitle") && (
          <section className="fortune-chart-editor-section">
            ...
            <CommitField
              label={tt.names.titleText}
              value={
                (element === "valueAxisTitle"
                  ? chart.valueAxisTitle
                  : chart.categoryAxisTitle) ?? ""
              }
              onCommit={(text) => {
                update((_ctx, c) => {
                  if (element === "valueAxisTitle") c.valueAxisTitle = text;
                  else c.categoryAxisTitle = text;
                });
                return undefined;
              }}
            />
  ...
  const renderSeries = (s: ChartSeries, i: number) => {
    ...
    const colorValue = /^#[0-9a-f]{6}$/i.test(color) ? color : "#4472c4";
  ...
  return (
    <SidePane id="format-chart" title={paneTitle} open onClose={close}>
      {/* Escape inside the pane closes it; the dock keeps keys from the grid */}
      {/* eslint-disable-next-line jsx-a11y/no-noninteractive-element-interactions */}
      <div
        ref={rootRef}
        className="fortune-chart-editor ts-pane-content"
        onKeyDown={(e) => {
          if (
            e.key === "Escape" &&
      ...
      >
        <div className="fortune-chart-editor-body">
          {elementSelect()}
          {element !== "chartArea" && elementPane()}
          {element === "chartArea" && (
            <>
              {formatSection("chartArea")}
              <section className="fortune-chart-editor-section">
                <label
                ...
                <ChartGalleries
                  ...
                  onStyle={(id) =>
                    update((_ctx, c) => {
                      ...
                      if (flags?.dataLabels != null)
                        c.dataLabels = flags.dataLabels;
              ...
              <section className="fortune-chart-editor-section">
                ...
                <button
                  ...
                  onClick={() =>
                    update((ctx, c) => switchChartRowColumn(ctx, c.id))
                  }
                >
                  {t.switchRowColumn}
                </button>
              ...
              <section className="fortune-chart-editor-section">
                ...
                <CommitField
                  ...
                  value={
                    categoriesRef
                      ? chartRangeToText(context, categoriesRef)
                      : ""
                  }
                  onCommit={(text) => {
                    const range = parseRange(text);
                ...
                />
                {chart.series.map((s, i) => renderSeries(s, i))}
                <button
                  type="button"
                  className="button-basic button-default"
                  disabled={readonly}
                  ...
                  {PLACEMENTS.map(([value, key]) => (
                    ...
                    >
                      <input
                        ...
                        onChange={() =>
                          update((ctx, c) =>
                            setChartPlacement(ctx, c.id, value)
                          )
                        }
                      />
                      {t[key]}
