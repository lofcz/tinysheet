...
export function recalculate(
  ...
  try {
    ...
    const iterate = !!calc?.iterate && calc.maxIterations > 1;
    // iterative calculation needs every formula evaluated first: no slicing
    done = executeAffectedFormulas(
      ctx,
      graph,
      order,
      data,
      iterate ? null : deadline
    );
    if (iterate && calc) {
