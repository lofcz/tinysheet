/**
 * Single-series bar renderer used for chart images (the xlsx importer's
 * fallback for charts that are not converted to live chart objects).
 */
...
export function renderEmptyChartSvg(width: number, height: number) {
  ...
  return (
    ...
    `<rect x="0" y="0" width="${svgWidth}" height="${svgHeight}" fill="#ffffff" stroke="#d9d9d9"/>` +
    `<text x="${svgWidth / 2}" y="${
      svgHeight / 2
...
export function renderBarChartSvg(
  ...
  let body =
    `<rect x="0" y="0" width="${svgWidth}" height="${svgHeight}" fill="#ffffff" rx="4" ry="4"/>` +
    `<rect x="0.5" y="0.5" width="${svgWidth - 1}" height="${
      svgHeight - 1
    }" fill="none" stroke="#d9d9d9" rx="4" ry="4"/>`;

  if (hasTitle) {
    body += `<text x="${roundSvgNumber(
      plotX + plotWidth / 2
    )}" y="${roundSvgNumber(
      Math.max(14, marginTop * 0.55)
    )}" text-anchor="middle" font-family="Arial, sans-serif" font-size="13" font-weight="600" fill="#333333">${escapeXml(
      options.title!.trim()
    )}</text>`;
  }
  ...
  if (hasValueAxisTitle) {
    ...
    body += `<text x="${roundSvgNumber(axisTitleX)}" y="${roundSvgNumber(
      axisTitleY
    )}" text-anchor="middle" font-family="Arial, sans-serif" font-size="11" fill="#666666" transform="rotate(-90 ${roundSvgNumber(
      axisTitleX
    )} ${roundSvgNumber(axisTitleY)})">${escapeXml(
      options.valueAxisTitle!.trim()
    )}</text>`;
  }
  ...
  for (let i = 0; i < ticks.length; i += 1) {
    ...
    body +=
      `<line x1="${roundSvgNumber(plotX)}" y1="${roundSvgNumber(
        y
      )}" x2="${roundSvgNumber(plotX + plotWidth)}" y2="${roundSvgNumber(
        y
      )}" stroke="#e6e6e6" stroke-width="1"/>` +
      `<text x="${roundSvgNumber(plotX - 6)}" y="${roundSvgNumber(
        y + 4
      )}" text-anchor="end" font-family="Arial, sans-serif" font-size="10" fill="#666666">${formatAxisTick(
        tickValue,
        scale.step
      )}</text>`;
  }
  ...
  body +=
    `<line x1="${roundSvgNumber(plotX)}" y1="${roundSvgNumber(
      plotY
    )}" x2="${roundSvgNumber(plotX)}" y2="${roundSvgNumber(
      plotY + plotHeight
  ...
  for (let i = 0; i < series.length; i += 1) {
    ...
    body += `<rect x="${roundSvgNumber(x)}" y="${roundSvgNumber(
      y
    )}" width="${roundSvgNumber(barWidth)}" height="${roundSvgNumber(
      Math.max(0, barHeight)
    )}" fill="${escapeXml(series[i].color)}"/>`;
  }
  ...
  if (hasCategoryAxisTitle) {
    body += `<text x="${roundSvgNumber(
      ...
      svgHeight - 10
    )}" text-anchor="middle" font-family="Arial, sans-serif" font-size="11" fill="#666666">${escapeXml(
      options.categoryAxisTitle!.trim()
    )}</text>`;
  }
  ...
  for (let i = 0; i < series.length; i += 1) {
    ...
    body +=
      `<rect x="${roundSvgNumber(legendX)}" y="${roundSvgNumber(
        y - 9
      )}" width="10" height="10" fill="${escapeXml(series[i].color)}"/>` +
      `<text x="${roundSvgNumber(legendX + 16)}" y="${roundSvgNumber(
        y
      )}" font-family="Arial, sans-serif" font-size="11" fill="#333333">${escapeXml(
        series[i].label
      )}</text>`;
  }

  return `<svg xmlns="http://www.w3.org/2000/svg" width="${svgWidth}" height="${svgHeight}" viewBox="0 0 ${svgWidth} ${svgHeight}">${body}</svg>`;
}
