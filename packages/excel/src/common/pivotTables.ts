 ...
 */
export async function addPivotTablesToZip(zip: JSZip, sheets: any[]) {
  if (!sheets.some((s) => s?.pivotTables?.length)) return;
  let workbookXml = await zip.file("xl/workbook.xml")?.async("string");
  let workbookRels = await zip
    .file("xl/_rels/workbook.xml.rels")
    ?.async("string");
  let types = await zip.file("[Content_Types].xml")?.async("string");
  if (!workbookXml || !workbookRels || !types) return;
...
/** Add the sheets' PivotTables to a written xlsx buffer. */
export async function addPivotTablesToXlsx(
  ...
  await addPivotTablesToZip(zip, sheets);
